...
map.on('load', () => {
    ...
    locations.forEach((location, i) => {
        if(!seenIcons.has(location.icon)) {
            seenIcons.add(location.icon);
            ...
            createImageBitmap(bitmap, 0, 0, bitmap.width, bitmap.height, {resizeWidth: 40, resizeHeight: 40}).then(resizedImage => {
                                map.addImage(location.icon, resizedImage);
                            });
                            addFeature(location, i);
            });
        }
        else
            addFeature(location, i);
...
var allFeaturesLoaded = function() {
...
var addFeature = function(location, i) {
    var feature = {
            type: 'feature',
            properties: {
                description:                    
        ...
        $('img[usemap]').mapster({
            ...
            onClick: function(e) {
                if (e.selected) {
                    ...
                    closeButton.classList.add('iframe-close-button');
...
$(document).on('click', '.iframe-close-button', function(e){
    $('iframe.fullPage').remove();
    $('.iframe-close-button').remove();
});
